...
/// Days with an unusually high number of screenshot creations. Thresholds
/// are fixed (see `atlas_core::timeline`), not user-configurable yet.
#[tauri::command]
pub fn get_screenshot_bursts(state: State<'_, AppState>) -> Result<Vec<Burst>, String> {
    let conn = state.db.lock();
    screenshot_bursts(&conn, SCREENSHOT_BURST_MIN_COUNT, BURST_SAMPLE_LIMIT)
        .map_err(|e| e.to_string())
}

/// Folder-and-day pairs with an unusually high number of file creations at
/// once. Thresholds are fixed (see `atlas_core::timeline`), not
/// user-configurable yet.
#[tauri::command]
pub fn get_project_bursts(state: State<'_, AppState>) -> Result<Vec<Burst>, String> {
    let conn = state.db.lock();
    project_bursts(&conn, PROJECT_BURST_MIN_COUNT, BURST_SAMPLE_LIMIT).map_err(|e| e.to_string())
}
