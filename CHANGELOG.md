...
- `atlas-ai` (new crate): `EmbeddingProvider`/`ChatProvider` traits (same seam pattern as `atlas_platform::PlatformFs`); `OllamaProvider` talking to a local Ollama instance over HTTP for embeddings and chat, degrading to a clear error rather than blocking anything when Ollama is not running or a model is missing; `CloudProvider`, an opt-in OpenAI-compatible chat client that only ever sends the caller's own typed text, never file names or paths; an embedding index over file names (category and containing folder for context, not extracted PDF/DOCX text) with cosine-similarity search as a linear scan in Rust; natural-language query translation into `atlas_search`'s existing filter DSL instead of raw SQL generation, validated against the real parser before being trusted, with a check that catches an LLM confusing a category name ("Document") with a file extension
- `atlas-db`: migration 0005 adds `file_embeddings` (per-file name vectors, model-scoped) and `ai_settings` (key-value AI configuration)
- `atlas-desktop`: `get_ai_status`, `get_ai_settings`, `set_ai_settings`, `build_search_index`/`cancel_search_index` (background job with `embed-progress`/`embed-finished` events, matching the scan/hash pattern), `semantic_search_files`, `translate_natural_language_query` (`use_cloud` re-checked against stored settings server-side, not trusted from the caller alone) commands
- UI: AI Search tab with two modes ("Ask in plain English", which shows the translated filter query before running it through the same results list M3 built, and "Semantic search", with its own match-score results list); status banner with a search-index build progress bar; a chat-model picker and a collapsed-by-default cloud AI settings panel; a per-request cloud confirmation dialog that a cloud-enabled setting alone does not bypass
- ADR 0011 (local AI layer scope): why translation targets the filter DSL rather than SQL (no new injection surface), why embeddings are scoped to file names rather than extracted document text, why cloud support is a generic OpenAI-compatible client, and why "smart suggestions" is satisfied by the translation UX itself rather than a new LLM-generated recommendation type (M5's rule-based recommendations already own that surface, deliberately without an LLM hallucination risk)
- Model download: `llama3.2:1b` (about 1.3 GB) pulled via Ollama with explicit user permission, specifically so the translation path could be verified against a real small chat model rather than only a scripted fake; `nomic-embed-text` was already installed
- 19 unit tests across the new crate, plus 4 tests marked `#[ignore]` that hit the real local Ollama instance (`cargo test -p atlas-ai -- --ignored`), verifying real HTTP calls, real embedding dimensionality (768, matching `nomic-embed-text`), and a real model's translation output surviving validation
- A real concurrency bug found by `cargo clippy`'s `significant_drop_tightening` lint, not by testing: an early draft of `get_ai_status` and `semantic_search_files` held the app's single shared database connection locked across the network call to Ollama, which would have frozen every other database-touching command for the duration of that request (up to a 60-second timeout). Fixed by splitting embedding-the-query (network) from ranking-by-similarity (pure SQL) into two separate calls so a connection is never held across a network request
- A second real gap caught by re-reading the flow before verification, not by a user report: `AiSettings.chat_model` had a backend field and command but nothing in the UI ever let a user set one, so natural-language translation would have silently fallen back to free text on every request forever. Fixed by adding a chat-model picker to the settings panel
- Two more real bugs found while verifying `translate()` against the real `llama3.2:1b` (not the scripted fakes the unit tests use): it wrote `type:zip, size>10mb` with a comma-space separator, which parsed "successfully" as the literal, never-matching extension `"zip,"` instead of failing; and `clean()`'s quote-unwrapping stripped a leading quote from a response that started with (but did not end with) a quoted phrase, breaking the phrase's pairing. Both fixed with regression tests; the system prompt was also tightened after observing the same small model repeatedly inventing age/folder filters nobody asked for
- Verified end-to-end against the real 315 GB / 26,790-file index: the status banner correctly reported live Ollama/model state throughout; Build search index showed real progress against real files and Cancel stopped it cleanly with the status banner reflecting the partial count afterward; Semantic search over a partial index (713 real files embedded) correctly ranked real project files together by meaning (a "video game files" query surfaced sprite images, background art, and a Makefile all from the same real game-project folder, highest-similarity first); the cloud confirmation dialog correctly showed the exact query text and target model before any send, and Cancel worked. Natural-language translation quality with `llama3.2:1b` specifically was inconsistent between runs of the same feature, ranging from clean and correct to inventing implausible filter syntax; every case still degraded safely (no crash, a transparent "Translated to:" preview, "No matches" rather than wrong matches) since the parser-validation step accepts only what is syntactically real, even when what is syntactically real is not semantically useful. This is a disclosed limitation of a very small local model, not a defect in the translation pipeline itself

#### M10. v1.0 release

- Autoupdater: `tauri-plugin-updater` + `tauri-plugin-process`, checking a `latest.json` manifest hosted on GitHub Releases; update payloads are minisign-signed with a real keypair generated via `tauri signer generate --ci`, the private half held only as the `TAURI_SIGNING_PRIVATE_KEY` GitHub Actions secret; `UpdateChecker` shows the available version and requires an explicit "Install & restart" click, never installs silently
- `vite.config.ts` bakes the real `package.json` version into the frontend bundle at build time (`__APP_VERSION__`) so the version shown in the UI can never drift from what was actually packaged
- `.github/workflows/release.yml` (new): builds and signs installers for Windows, macOS, and Linux via `tauri-apps/tauri-action` on a `v*` tag push, publishing a **draft** GitHub Release with `latest.json` attached; a human still has to review and publish it, matching how this project treats every other hard-to-reverse action
- `.github/workflows/build.yml`: no longer triggers on tag pushes (that is now `release.yml`'s job only); stays the per-push/PR "does the bundle still build" check it always was, with no secrets involved
- `.github/workflows/pages.yml` (new): deploys `landing/` to GitHub Pages on any change to it
- `landing/`: a static HTML/CSS landing page (no framework, no build step) reusing the desktop app's own color tokens, covering principles, shipped features, and the full M0-M10 build-in-the-open history
- ADR 0012 (v1.0 release pipeline): the updater signing approach and why the key is intentionally passwordless (CI-secret-protected, standard practice); why real OS code-signing (Windows Authenticode, Apple notarization) is deferred and documented rather than faked, since both need a paid certificate and identity verification that cannot be completed on the user's behalf; why releases stage as drafts instead of auto-publishing; why `build.yml` and `release.yml` stay separate; why a demo video is out of scope for this release (no `ffmpeg` or screen-recording tool available in this environment) in favor of real static screenshots
- Known, disclosed limitation: installers are not yet signed with a paid OS-level publisher certificate, so Windows/macOS may show an unknown-publisher warning on first install; the updater's own minisign signature is unaffected by this and still verifies every update payload
